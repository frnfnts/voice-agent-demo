...
def build_exit_interview_graph() -> StateGraph:
    """退職面談の StateGraph を構築して返す（compile はしない）."""
    ...
    graph.add_node("evaluate", make_evaluate_node(EXIT_INTERVIEW_STEPS))
    graph.add_node("closing", make_closing_node())

    # ── エッジ登録 ──
    graph.set_entry_point("greet")
    graph.add_edge("greet", "ask")
    graph.add_edge("ask", "listen")       # interrupt_after="ask" でここで一時停止
